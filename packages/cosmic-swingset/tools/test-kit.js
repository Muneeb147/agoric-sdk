...
export const makeCosmicSwingsetTestKit = async (
  ...
  let lastBlockMonotime = Date.now();
  ...
  const nextBlockTime = () => {
    const delta = Math.floor(Date.now() - lastBlockMonotime);
    return lastBlockTime + (delta > 0 ? delta : 1);
  };

  if (!slogSender && (env.SLOGFILE || env.SLOGSENDER || env.SLOGSENDER_AGENT)) {
